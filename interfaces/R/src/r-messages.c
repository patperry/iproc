...
static ssize_t *copy_sexp_to_int64(struct array *dst, SEXP Rsrc)
{
	int i, n = GET_LENGTH(Rsrc);
	int *src = INTEGER_POINTER(Rsrc);
	int s;

	array_add_range(dst, NULL, n);
	...
	for (i = 0; i < n; i++) {
		...
		*(ssize_t *)array_item(dst, i) = s - 1;
	}

	if (n > 0) {
		return array_item(dst, 0);
	} else {
		return NULL;
	}
}
...
SEXP Riproc_messages_new(SEXP Rtime, SEXP Rfrom, SEXP Rto)
{
	...
	array_init(&to_buf, sizeof(ssize_t));
