#ifndef CATDIST1_H
#define CATDIST1_H

#include <assert.h>
#include <stdlib.h>
#include "catdist.h"

struct catdist1 {
	const struct catdist *parent;

	double *deta;
	size_t *ind;
	size_t nz, nzmax;
};

void catdist1_init(struct catdist1 *m1, const struct catdist *parent);
void catdist1_deinit(struct catdist1 *m1);
void catdist1_clear(struct catdist1 *m1);

static inline size_t catdist1_ncat(const struct catdist1 *m1);

double catdist1_prob(const struct catdist1 *m1, size_t i);
double catdist1_lprob(const struct catdist1 *m1, size_t i);

double catdist1_eta(const struct catdist1 *m1, size_t i);
double catdist1_deta(const struct catdist1 *m1, size_t i);
void catdist1_set_deta(struct catdist1 *m1, size_t i, double deta);
static inline void catdist1_get_deta(const struct catdist1 *m1,
				    const size_t **ind, const double **deta,
				    size_t *nz);

double catdist1_psi(const struct catdist1 *m1);
double catdist1_dpsi(const struct catdist1 *m1);

int _catdist1_check(const struct catdist1 *m);

size_t catdist1_ncat(const struct catdist1 *m1)
{
	return catdist_ncat(m1->parent);
}

void catdist1_get_deta(const struct catdist1 *m1,
		      const size_t **ind,
		      const double **deta,
		      size_t *nz)
{
	*ind = m1->ind;
	*deta = m1->deta;
	*nz = m1->nz;
}

#endif /* CATDIST1_H */
