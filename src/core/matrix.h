...
/* views */
static inline struct matrix matrix_make(const double *v, ssize_t m,
					ssize_t n);
static inline struct matrix matrix_make_with_lda(const double *v,
						 ssize_t m, ssize_t n,
						 ssize_t lda);
...
static inline struct matrix matrix_slice_cols(const struct matrix *a, ssize_t j,
					      ssize_t n);
static inline double *matrix_col(const struct matrix *a, ssize_t j);
...
struct matrix matrix_make(const double *v, ssize_t m, ssize_t n)
{
	return matrix_make_with_lda(v, m, n, MAX(1, m));
}

struct matrix matrix_make_with_lda(const double *v, ssize_t m, ssize_t n,
				   ssize_t lda)
{
	assert(m >= 0);
	assert(n >= 0);
	assert(lda >= MAX(1, m));
	assert(n <= SIZE_MAX / lda);
			   ...
			   ssize_t m, ssize_t n)
{
	...
	return matrix_make_with_lda(ptr, m, n, lda);
}
...
double *matrix_col(const struct matrix *a, ssize_t j)
{
...
double *matrix_item_ptr(const struct matrix *a, ssize_t i, ssize_t j)
{
	...
	return a->data + i + j * matrix_lda(a);
}
