...
ssize_t array_find_index(const struct array *a, predicate_fn match, void *udata)
{
	...
	return forward_find_index(array_item(a, 0), array_count(a), match, udata,
				  array_elt_size(a));
}
...
ssize_t array_find_last_index(const struct array *a, predicate_fn match, void *udata)
{
	...
	return reverse_find_index(array_item(a, 0), array_count(a), match, udata,
				  array_elt_size(a));
}
...
void *array_insert_range(struct array *l, ssize_t i, const void *vals, ssize_t n)
{
	assert(l);
	assert(i >= 0);
...
void array_sort(struct array *a, compare_fn compar)
{
	...
	qsort(array_item(a, 0), array_count(a), array_elt_size(a), compar);
}
