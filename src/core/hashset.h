...
static inline int hashset_equals(const struct hashset *s, const void *val1,
				 const void *val2);

/* modification */
bool hashset_contains(const struct hashset *s, const void *key);
void *hashset_lookup(const struct intmap *m, const void *key);
void *hashset_lookup_with(const struct intmap *m, const void *key,
			  const void *val0);
...
ssize_t hashset_buckets(const struct hashset *s)
{
	return intmap_size(&s->buckets);
}

bool hashset_empty(const struct hashset * s)
{
	return intmap_empty(&s->buckets);
}
...
{
	return intmap_max_size(&s->buckets);
}
