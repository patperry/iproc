...
static inline const intptr_t *intmap_keys_ptr(const struct intmap *m,
					      ssize_t i);
...
{
	return intset_size(&m->keys);
}

bool intmap_empty(const struct intmap * m)
{
	return intset_empty(&m->keys);
}

ssize_t intmap_max_size(const struct intmap * m)
{
	return MIN(intset_max_size(&m->keys), darray_max_size(&m->vals));
}

size_t intmap_elt_size(const struct intmap * m)
{
	return darray_elt_size(&m->vals);
}

intptr_t intmap_min_key(const struct intmap * m)
{
	return intset_min(&m->keys);
}

intptr_t intmap_max_key(const struct intmap * m)
{
	return intset_max(&m->keys);
}

intptr_t intmap_key_at(const struct intmap * m, ssize_t i)
{
	return intset_at(&m->keys, i);
}

void *intmap_vals_begin(const struct intmap *m)
{
	return darray_begin(&m->vals);
}

void *intmap_vals_end(const struct intmap *m)
{
	return darray_end(&m->vals);
}

void *intmap_vals_ptr(const struct intmap *m, ssize_t i)
{
	return darray_ptr(&m->vals, i);
}

const intptr_t *intmap_keys_begin(const struct intmap *m)
{
	return intset_begin(&m->keys);
}

const intptr_t *intmap_keys_end(const struct intmap *m)
{
	return intset_end(&m->keys);
}

const intptr_t *intmap_keys_ptr(const struct intmap *m, ssize_t i)
{
	return intset_ptr(&m->keys, i);
}
