...
static void nsend_frame_init(struct frame_var *fv, struct frame *f)
{
	...
	struct nsend_udata *udata = xcalloc(1, sizeof(*udata));
	array_init(&udata->active, sizeof(struct nsend_active));

	ssize_t i, n = design_recv_count(d);
	for (i = 0; i < n; i++) {
		struct nsend_active *active = array_add(&udata->active, NULL);
...
static void nsend_frame_deinit(struct frame_var *fv)
{
	...
	struct nsend_udata *udata = fv->udata;
	struct nsend_active *active;
...
static void nsend_frame_clear(struct frame_var *fv)
{
	assert(fv);
	assert(fv->udata);
...
static void nsend_handle_event(struct frame_var *fv,
			       const struct frame_event *e, struct frame *f)
{
	...
	if (e->type == DYAD_EVENT_INIT) {
		struct nsend_udata *udata = fv->udata;
		struct nsend_active *active =
		    array_item(&udata->active, meta->msg_dyad.isend);
		if (intset_add(&active->jrecv, meta->msg_dyad.jrecv)) {
			dx.type = RECV_VAR_EVENT;
			dx.time = e->time;
			dx.id = -1;
			dx.meta.recv_var.item.isend = meta->msg_dyad.isend;
			dx.meta.recv_var.item.jrecv = meta->msg_dyad.jrecv;
