 ...
 */
void
iproc_logsumexp_init (iproc_logsumexp *lse)
{
    assert(lse);
    lse->max = -INFINITY;
    lse->sumexpm1 = -1.0;
...
void
iproc_logsumexp_insert (iproc_logsumexp *lse,
                        double           val)
{
    assert(lse);
...
double
iproc_logsumexp_value (iproc_logsumexp *lse)
{
    assert(lse);
...
double
iproc_logsumexp_max (iproc_logsumexp *lse)
{
    assert(lse);
    return lse->max;
}
