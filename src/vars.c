...
char **var_names_alloc(char *name, size_t len, ssize_t n)
{
	...
	for (i = 0; i < n; i++) {
		ssize_t digits = 1 + (i + 1) / 10;
		size_t len1 = len + 2 + digits + 1;
		res[i] = xcalloc(len1, sizeof(res[i][0]));
		size_t nout = snprintf(res[i], len1, "%s(%"SSIZE_FMT")", name, i + 1);
		assert(nout + 1 == len1);
		(void)nout;
	}
	res[n] = NULL;
...
char **var_names_alloc2(char *name, size_t len, ssize_t m, ssize_t n)
{
	...
	for (j = 0; j < n; j++) {
		...
		for (i = 0; i < m; i++) {
			ssize_t idigits = 1 + (i + 1) / 10;
			size_t len1 = len + 3 + idigits + jdigits + 1;
			
			ix = i + j * m;
			res[ix] = xcalloc(len1, sizeof(res[ix][0]));
			size_t nout = snprintf(res[ix], len1, "%s(%"SSIZE_FMT",%"SSIZE_FMT")", name, i + 1, j + 1);
			assert(nout + 1 == len1);
			(void)nout;
		}
	}
